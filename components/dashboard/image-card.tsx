...
import { Calendar, AlertCircle, Trash2, Star, ImagePlus } from "lucide-react";
...
import { toggleImagePublic } from "@/actions/image/toggle-image-public";
...
interface ImageCardProps {
  image: ImageWithProductName;
  onDelete?: () => void;
  onPublicToggle?: () => void;
}

/**
 * Image card component for dashboard grid
 * Displays thumbnail, product name, status, and action button
 */
export function ImageCard({ image, onDelete, onPublicToggle }: ImageCardProps) {
  ...
  // Format date
  const createdDate = new Date(image.created_at).toLocaleDateString("ko-KR", {
    year: "numeric",
    month: "long",
    day: "numeric",
  });
  ...
  const handleDelete = async () => {
    try {
      ...
      if (!result.success) {
        toast.error(result.error || "이미지 삭제 중 오류가 발생했습니다.");
        return;
      }
  ...
  const handleTogglePublic = async () => {
    try {
      ...
      const result = await toggleImagePublic(image.id, newIsPublic);
  ...
  return (
    <Card className="overflow-hidden hover:shadow-lg transition-shadow duration-200">
      {/* Thumbnail - 1:1 aspect ratio for images */}
      <Link href={linkHref} className="block relative aspect-square bg-muted">
        {!imageError && image.image_url ? (
          <Image
            src={image.image_url}
            alt={image.product_name || "광고 이미지"}
            fill
      ...
      <CardFooter className="pt-0 pb-4 flex gap-2">
        <Button asChild variant="outline" size="sm" className="flex-1">
          <Link href={linkHref}>
            {image.status === "completed" ? "이미지 보기" : "진행 상태 보기"}
          </Link>
        </Button>

        {/* Public toggle button - only show for completed images */}
        {image.status === "completed" && (
          <Button
            variant="outline"
            size="sm"
        ...
        <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                이 작업은 되돌릴 수 없습니다. 이미지와 관련된 모든 데이터가 영구적으로 삭제됩니다.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
